...
分布式同步锁机制：
1. **GitHub 锁文件** `.sync-in-progress` - 存储设备 ID、开始时间、TTL
2. **最近 commit 时间检查** - 阈值内有人同步则跳过
3. **随机 Jitter 等待** - 避免多设备同时通过检查
4. **双重检查模式** - Jitter 后再次检查 commit 时间
5. **锁 TTL 自动过期** - 防止死锁（默认 10 分钟）

详见 TECHNICAL-ARCHITECTURE.md 的"v0.4.3 分布式同步锁机制"章节。